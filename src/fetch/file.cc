...
void html::parseTag () {
  ...
  if (thisCharIs(0, 'a')) { // a href
    ...
    isTag(thisCharIs(1, 'c') && thisCharIs(2, 'r') && thisCharIs(3, 'i') && thisCharIs(4, 'p') &&thisCharIs(5, 't'), "src", LINK, ttScript, 6);
  } else if (thisCharIs(0, 'l')) { // link href
    ...
    isTag(thisCharIs(1, 'r') && thisCharIs(2, 'a')
          ...
          "src", LINK, ttFrame, 5);
  } else if (thisCharIs(0, 'i') && thisCharIs(1, 'f')) { // iframe src
    isTag(thisCharIs(2, 'r') && thisCharIs(3, 'a') && thisCharIs(4, 'm') && thisCharIs(5, 'e'),
          "src", LINK, ttIFrame, 6);
#ifdef IMAGES
  } else if (thisCharIs(0, 'i') && thisCharIs(1, 'm')) { // img src
    isTag(thisCharIs(2, 'g'), "src", LINK, ttImage, 3);
#endif // IMAGES
  } else {
    return;
  }
...
void html::parseContent (int action, TagType tagType) {
  posParse++;
  ...
  if (posParse == buffer + pos) {
    ...
    *posParse = 0;
    ...
    case LINK:
      ...
      manageUrl(new url(area, here->getDepth()-1, base), tagType, false);
      break;
    case BASE:
