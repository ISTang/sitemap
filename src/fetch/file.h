...
struct LinkInfo
{
  ...
  LinkInfo(char *url, TagType type) {
    this->url = url;
    this->type = type;
  }
};
...
class html : public file {
 private:
  ...
  void manageUrl (url *nouv, TagType tagType, bool isRedir);
  ...
  void parseTag ();
  // enter a tag content
  void parseContent (int action, TagType tagType);
...
#ifdef LINKS_INFO
  /** return the links */
  inline Vector<LinkInfo> *getLinks () { return &links; }
#endif // LINKS_INFO
};
