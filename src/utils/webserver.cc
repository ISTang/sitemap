...
void *startWebserver(void *none) {
	...
	if ((fds = socket(AF_INET, SOCK_STREAM, 0)) == -1
			|| setsockopt(fds, SOL_SOCKET, SO_REUSEADDR, (char*) &nAllowReuse,
					sizeof(nAllowReuse))
			|| bind(fds, (struct sockaddr *) &addr, sizeof(addr)) != 0
			|| listen(fds, 4) != 0) {
		syslog(LOG_ERR, "无法为 web 服务器获取套接字");
		exit(1);
	}
	// answer requests
	for (;;) {
		...
		if (fdc == -1) {
			syslog(LOG_WARNING, "Web 服务器遇到了麻烦...");
		} else {
...
static void writeStats(int fds) {
	...
	ecrire(fds, "\n\n就绪站点 :                 ");
	ecrireInti(fds,
			global::okSites->getLength() + global::nb_conn
...
static void writeUrls(int fds) {
	...
	if (ds != NULL) {
		...
		for (uint i = ds->outFifo; i != ds->inFifo;
				i = (i + 1) % maxUrlsBySite) {
